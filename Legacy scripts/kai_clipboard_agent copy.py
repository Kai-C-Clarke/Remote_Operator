...
class KaiClipboardAgent(KaiAgent):
    ...
    def find_claude_input_area(self):
        """Find and click on Claude's input area"""
        try:
            # Get screen dimensions
            screen_width, screen_height = pyautogui.size()
            
            # Claude input is typically at bottom center of screen
            input_x = screen_width // 2
            input_y = screen_height - 150  # Near bottom
            
            self.log(f"Clicking Claude input area at ({input_x}, {input_y})")

...
class KaiCoordinator:
    ...
    def run_full_automation_cycle(self):
        """Complete automation cycle using clipboard copy method"""
        ...
        try:
            ...
            if stable_text and '<<' in stable_text and '>>' in stable_text:
                self.logger.info("Found boundary markers, attempting to copy URL directly...")
                
                # Take a fresh screenshot of Claude's region
                ...
                if screenshot_path:
                    try:
                        # Load image and use OCR to find URL coordinates
                        from PIL import Image
                        import pytesseract
                        ...
                        # Look for URL-like text
                        url_found = False
                        for i, text in enumerate(data['text']):
                            if text and ('http' in text.lower() or 'bbc' in text.lower() or 'www' in text.lower()):
                                # Found URL-like text, get coordinates
                                x = data['left'][i] + text_region[0]  # Adjust for region offset
                                y = data['top'][i] + text_region[1]
                                w = data['width'][i]
                                
                                # Click and select the URL
                                click_x = x + w // 2
                                click_y = y
                                
                                self.logger.info(f"Found URL text at ({click_x}, {click_y}): {text}")
                                self.logger.info("Triple-clicking to select URL line")
                                
                                pyautogui.click(click_x, click_y, clicks=3)
                                time.sleep(0.3)
                    ...
                    self.logger.error("Failed to capture Claude region for URL copying")
                    return False
            else:
                self.logger.error("No boundary markers found in Claude's response")
                return False

            # Step 3: Navigate to URL using clipboard
            desktop_agent = KaiDesktopAgent(target_desktop=2)
            desktop_agent.run_with_retry()
            self.state['current_desktop'] = 2

            web_agent = KaiWebAgent(use_clipboard=True)
            if not web_agent.run_with_retry():
                self.logger.error("Failed to navigate using clipboard URL")
                return False

            # Step 4: Wait for page load and send confirmation
            time.sleep(4)

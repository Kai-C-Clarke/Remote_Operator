...
class KaiClipboardAgent(KaiAgent):
    ...
    def run(self):
        ...
        # Log the full message being copied
        self.log(f"Copying full message to clipboard ({len(self.message)} chars): {self.message}")
        
        # Use pyperclip to copy the full message
        pyperclip.copy(self.message)
        time.sleep(1)  # Longer wait for clipboard

        # Verify clipboard content
        clipboard_content = pyperclip.paste()
        self.log(f"Clipboard verification: {len(clipboard_content)} chars copied")
        
        if clipboard_content != self.message:
            self.log(f"Clipboard mismatch! Expected: {self.message[:50]}...")
            self.log(f"Got: {clipboard_content[:50]}...")
        ...
        try:
            ...
            time.sleep(1)  # Longer wait for paste
        except Exception as e:

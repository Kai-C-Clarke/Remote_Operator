    ...
    def run_full_automation_cycle(self):
        ...
        try:
            ...
            if stable_text and '<<' in stable_text and '>>' in stable_text:
                # Found boundary markers, now try to copy URL directly
                url_copied = boundary_detector.find_and_copy_url_from_boundaries()
                
                if url_copied:
                    self.logger.info("URL successfully copied to clipboard")
                else:
                    self.logger.error("Failed to copy URL from screen")
                    return False
            else:

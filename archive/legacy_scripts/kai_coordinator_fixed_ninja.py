#!/usr/bin/env python3
"""
KaiCoordinator - Final Fixed Version 1.2
=========================================
Date: September 8, 2025
Changes in v1.2:
- Fixed class name mismatch (KaiEnhancedWebAgent -> KaiWebAgent)
- Added missing run_with_url_variable method to KaiWebAgent
- Fixed extracted_url variable handling
- Removed duplicate confirmation code block
- Fixed logging tag in KaiWebAgent
...
class KaiWebAgent:
    ...
    def log(self, message):
        self.logger.info(f"[KaiWebAgent] {message}")  # Fixed logging tag

    def run_with_retry(self):
        """This method is called by the main cycle - redirect to variable-based method"""
        # This method exists for backward compatibility but should not be used
        # The main cycle should call run_with_url_variable() directly
        self.log("WARNING: run_with_retry() called instead of run_with_url_variable()")
        self.log("This indicates the main cycle is using the old method call")
        return False  # Force failure to highlight the issue

    def run(self):
        """DEPRECATED: This method should not be used - use run_with_url_variable() instead"""
        self.log("ERROR: Deprecated run() method called - this should use run_with_url_variable()")
        return False
        
    # Added missing method to handle URL variable
    def run_with_url_variable(self, url=None):
        """Method to navigate to a URL passed as a variable or from clipboard"""
        if url:
            self.log(f"Navigating to provided URL: {url}")
        else:
            # Get URL from clipboard if not provided
            try:
                clipboard_result = subprocess.run(['pbpaste'], capture_output=True, text=True)
                url = clipboard_result.stdout.strip()
                self.log(f"Using URL from clipboard: {url}")
            except Exception as e:
                self.log(f"Failed to get URL from clipboard: {e}")
                return False
        
        # Try keyboard method first, then AppleScript as fallback
        return self._try_keyboard_method() or self._try_applescript_method()
...
class KaiCoordinator:
    ...
    def run_full_automation_cycle(self):
        ...
        try:
                    ...
                    try:
                        ...
                        for i, text in enumerate(data['text']):
                            if text and ('http' in text.lower() or 'bbc' in text.lower() or 'www' in text.lower()):
                                ...
                                # Get the URL from clipboard to store in state
                                clipboard_result = subprocess.run(['pbpaste'], capture_output=True, text=True)
                                extracted_url = clipboard_result.stdout.strip()
                                self.state['last_url'] = extracted_url
                                self.logger.info(f"Extracted URL from clipboard: {extracted_url}")
            ...
            # FIXED: Use correct web agent class and no need to pass URL
            web_agent = KaiWebAgent()
            if not web_agent.run_with_url_variable():
...
if __name__ == "__main__":
    print("KaiCoordinator - Version 1.2 (Fixed Class Names and URL Handling)")
    coordinator = KaiCoordinator()
    
    import argparse
    parser = argparse.ArgumentParser(description='Kai Automation System v1.2')

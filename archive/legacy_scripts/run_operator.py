...
def ask_for_url():
    """Prompt Claude to provide a URL in bold format"""
    starter_message = (
        "Please provide a real webpage URL in bold format using **URL** syntax. "
...
def open_and_capture(url):
    ...
    # FIXED: Switch to Desktop 2 BEFORE opening browser
    print("🔄 Switching to Desktop 2 for browser...")
    desktop_agent = KaiDesktopAgent(target_desktop=1)  # Desktop 1 = your Desktop 2
    desktop_agent.run()
    time.sleep(1)
    ...
    # FIXED: Switch back to Desktop 1 (Claude's desktop) BEFORE pasting
    print("🔄 Switching back to Claude's desktop...")
    desktop_agent_return = KaiDesktopAgent(target_desktop=0)  # Desktop 0 = your Desktop 1
    desktop_agent_return.run()
    time.sleep(2)  # Extra time for desktop switch

    # FIXED: Use precise input area clicking before pasting
    ...
    try:
        ...
        # Use your measured coordinates for Claude's input
        claude_input_x = 1845  # Center of input area
        claude_input_y = 1280
        ...
        print(f"Failed to paste screenshot: {e}")
        return False
...
def main():
    ...
    desktop_agent = KaiDesktopAgent(target_desktop=0)  # Desktop 0 = your Desktop 1
    desktop_agent.run()

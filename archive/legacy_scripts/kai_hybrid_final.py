...
class KaiWebAgent:
    ...
    def run_with_url_variable(self, url=None):
        """Primary method: Navigate using variable or clipboard with advanced fallback"""
        if url:
            ...
            try:
                ...
                self.extracted_url = clipboard_result.stdout.strip()
                self.log(f"Retrieved URL from clipboard: {self.extracted_url}")
            except Exception as e:
        ...
        if not self.extracted_url or not (
            ...
            any(domain in self.extracted_url.lower() for domain in ['bbc', 'wikipedia', 'google'])
        ):
            self.log(f"Invalid URL format: {self.extracted_url}")
            return False
        
        # Try multiple navigation methods with fallback
        methods = [
            ("keyboard shortcut", self._try_keyboard_method),
            ("AppleScript", self._try_applescript_method),
            ("direct launch", self._try_direct_launch_method)
        ]
        
        for method_name, method_func in methods:
            self.log(f"Attempting {method_name} method...")
            try:
                if method_func():
                    self.log(f"Success: {method_name} method worked")
                    return True
                else:
                    self.log(f"Failed: {method_name} method unsuccessful")
            except Exception as e:
                self.log(f"Error in {method_name} method: {e}")
        
        self.log("All navigation methods failed")
        return False
        ...
        try:
            ...
            for i, text in enumerate(data['text']):
                if text and any(keyword in text.lower() for keyword in ['http', 'www', 'bbc', 'wikipedia', 'google']):
                    ...
                    # Use drag selection to avoid link trigger
                    start_x = x - 10
                    end_x = x + w + 10
                    click_y = y + 5
                    ...
                    time.sleep(0.2)
                    pyautogui.dragTo(end_x, click_y, duration=0.5, button='left')
                    time.sleep(0.3)
